...
// TEMP DATA SO IT WORKS WITHOUT API - DELETE LATER
const MOCK_TOOLS = [
  { slug: 'chatgpt', name: 'ChatGPT', rating: 4.8, tagline: 'OpenAI flagship conversational AI', color: 'from-green-500 to-emerald-500', initial: 'C', pricing: 'Freemium', category: 'Chatbots & Assistants', views: 982341 },
  { slug: 'claude', name: 'Claude', rating: 4.9, tagline: 'Anthropic safety-first AI assistant', color: 'from-orange-500 to-red-500', initial: 'C', pricing: 'Freemium', category: 'Chatbots & Assistants', views: 612400 },
  { slug: 'midjourney', name: 'Midjourney', rating: 4.9, tagline: 'Stunning AI art generation', color: 'from-blue-500 to-indigo-500', initial: 'M', pricing: 'Paid', category: 'Image Generation', views: 754300 },
  { slug: 'cursor', name: 'Cursor', rating: 4.9, tagline: 'The AI code editor', color: 'from-cyan-500 to-blue-500', initial: 'C', pricing: 'Freemium', category: 'Coding & Dev', views: 511000 },
];

const MOCK_CATEGORIES = [
  { slug: 'chatbots', name: 'Chatbots & Assistants', icon: 'MessageSquare', gradient: 'from-green-500 to-emerald-500', count: 5, description: 'Conversational AI' },
  { slug: 'image', name: 'Image Generation', icon: 'Image', gradient: 'from-purple-500 to-pink-500', count: 6, description: 'AI art tools' },
  { slug: 'coding', name: 'Coding & Dev', icon: 'Code2', gradient: 'from-blue-500 to-cyan-500', count: 7, description: 'AI for developers' },
...
function useFetch(url: string) { 
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (alive) {
        if (url.includes('trending')) setData({ tools: MOCK_TOOLS });
        if (url.includes('categories')) setData({ categories: MOCK_CATEGORIES });
        if (url.includes('stats')) setData({ tools: 46, categories: 12 });
        setLoading(false);
      }
    }, 500);
    return () => { alive = false; };
